...
import { getMeasurementsForCustomer } from "@/lib/measurements";
...
export default async function CustomerDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ created?: string; updated?: string }> }) {
  ...
  return (
    <div className="space-y-7">
      ...
      <section aria-labelledby="customer-measurements-title"><div className="mb-3"><h2 id="customer-measurements-title" className="text-xl font-bold text-slate-950">Recent Measurements</h2><p className="mt-1 text-sm text-slate-500">Measured zones across this customer’s properties.</p></div>{measurements.length ? <Card className="overflow-hidden"><div className="divide-y divide-slate-100">{measurements.map((measurement) => { const measuredProperty = customer.properties.find((property) => property.id === measurement.property_id); return <div key={measurement.id} className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="flex gap-3"><span className="flex size-9 shrink-0 items-center justify-center rounded-lg bg-cyan-100 text-cyan-700"><Ruler className="size-4" /></span><div><p className="font-bold text-slate-950">{measurement.zone_name}</p><p className="mt-1 text-sm text-slate-600">{measurement.measurement_type} · {Number(measurement.quantity).toLocaleString("en-US", { maximumFractionDigits: 2 })} {measurement.unit}</p><p className="mt-1 text-xs text-slate-500">{measuredProperty?.property_name || measuredProperty?.address_line_1 || "Property"} · {formatVisitDate(measurement.visit_date)}</p></div></div><LinkButton href={`/measurements/${measurement.id}`} variant="secondary" size="sm">View</LinkButton></div>; })}</div></Card> : <EmptyState compact icon={<Ruler className="size-5" />} title="No measurements yet" description="Measurements will appear after a site visit is measured." />}</section>
