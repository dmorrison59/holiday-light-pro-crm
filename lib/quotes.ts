...
export async function getQuotes(organizationId: string, filters: { search?: string; status?: string } = {}) {
  const supabase = await createSupabaseServerClient(); let query = supabase.from("quotes").select("*").eq("organization_id", organizationId); if (filters.status === "pending") query = query.in("status", ["draft", "sent"]); else if (filters.status) query = query.eq("status", filters.status); const { data, error } = await query.order("quote_date", { ascending: false }).order("created_at", { ascending: false }); if (error) throw new Error("We could not load quotes."); let records = await attachQuoteRelations(organizationId, data ?? []); const search = filters.search?.trim().toLocaleLowerCase(); if (search) records = records.filter((item) => [item.quote_number, item.customer.first_name, item.customer.last_name, item.property.property_name, item.property.address_line_1, item.property.city].filter(Boolean).join(" ").toLocaleLowerCase().includes(search)); return records;
}
