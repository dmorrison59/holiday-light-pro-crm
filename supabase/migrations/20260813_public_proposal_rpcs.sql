...
begin
  ...
  where q.proposal_token = p_token
    ...
    and (
      q.site_visit_id is null
      or exists (
        select 1
        from public.site_visits sv
        where sv.id = q.site_visit_id
          and sv.organization_id = q.organization_id
          and sv.customer_id = q.customer_id
          and sv.property_id = q.property_id
      )
    )
    and (
      q.package_id is null
      or exists (
        select 1
        from public.packages pkg
        where pkg.id = q.package_id
          and pkg.organization_id = q.organization_id
      )
    );
...
-- Permissions
-- ---------------------------------------------------------------------------
revoke all on function public.is_valid_proposal_token(text) from public, anon, authenticated;
