...
export function MeasurementForm({ siteVisit, catalogItems, measurement }: { siteVisit: SiteVisitRecord; catalogItems: MeasurementCatalogItem[]; measurement?: MeasurementRecord }) {
  ...
  return <form action={action} className="space-y-6"><input type="hidden" name="site_visit_id" value={siteVisit.id} />{measurement ? <input type="hidden" name="measurement_id" value={measurement.id} /> : null}<FormMessage error={state.error} />
    ...
    <div className="grid gap-5 sm:grid-cols-2"><FormField label="Measurement type" required><Select name="measurement_type" required defaultValue={measurement?.measurement_type ?? "Roofline"}>{measurementTypes.map((type) => <option key={type} value={type}>{type}</option>)}</Select></FormField><FormField label="Unit" required><Select name="unit" required defaultValue={measurement?.unit ?? "ft"}>{measurementUnits.map((unit) => <option key={unit} value={unit}>{unit}</option>)}</Select></FormField></div>
    <div className="grid gap-5 sm:grid-cols-2"><FormField label="Quantity" required><Input name="quantity" type="number" required min="0.01" step="0.01" inputMode="decimal" defaultValue={measurement?.quantity ?? "1"} /></FormField><FormField label="Height level"><Select name="height_level" defaultValue={measurement?.height_level ?? "Ground level"}>{heightLevels.map((level) => <option key={level} value={level}>{level}</option>)}</Select></FormField></div>
    ...
    <FormField label="Notes"><Textarea name="notes" defaultValue={measurement?.notes ?? ""} className="min-h-36" placeholder="Access notes, customer preferences, ladder position, or installation details." /></FormField>
